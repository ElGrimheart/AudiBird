...
def station_startup():
    """Initializes the station by monitoring its state and starting the detection service once station is configured."""

    # Check if this is the first run and handle announcement to backend if required
    if (is_first_run):
        announce_station()
        config_check = get_station_config()
        if not config_check['station']['id'] or not config_check['station']['api_key']:
            print("Failed to announce station. Please check network connections and retry. Exiting...")
            sys.exit(1)
    ...
    while True:
        ...
        match current_state:
            case "announced":
                ...
                print(f"Station has not been registered. Please register through the web application using the station ID: {station_id} and API Key: {station_api_key}")
            case "claimed":
                print("Station awaiting configuration. Please finish configuring the station through the web application.")
            case "configured":
...
def fetch_latest_config():
    """Fetch the latest config from the backend and update the local config file."""
    config = get_station_config()
    station_id = config['station'].get('id')
    api_key = config['station'].get('api_key')

    if not station_id or not api_key:
        print("Missing station ID or API key. Cannot fetch latest config.")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if latest_config:
                ...
                print("Fetched and updated local config from backend.")
                return True
            else:
                print("No config found in backend response.")
        else:
